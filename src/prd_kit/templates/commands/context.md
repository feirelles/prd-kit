...
1. **Setup**: Run setup script to get paths:
   ```bash
   cd "$(git rev-parse --show-toplevel 2>/dev/null || echo "ERROR: Run from git repository root")/.prd-kit/scripts" && python -m prd_scripts.setup_context --spec "[spec-identifier]" --json
   ```
