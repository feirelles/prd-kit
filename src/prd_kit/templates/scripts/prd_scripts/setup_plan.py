#!/usr/bin/env python3
"""PRD Kit - Plan Phase Setup Script.

Sets up environment for technical plan generation.
"""
...
def main(args: list[str] | None = None) -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="PRD Kit Plan Setup")
    ...
    plan_template = paths.templates_dir / "plan-template.md"
    command_file = paths.commands_dir / "plan.md"
    ...
    if parsed.json:
        output = {
            ...
            "PLAN_TEMPLATE": str(plan_template),
        ...
        log_info(f"Plan: {'exists' if has_plan else 'to be created'}")
        log_info(f"Tech Constitution: {'✓' if has_tech_constitution else '✗'}")
        
        if has_context:
            log_success("Plan phase ready")
        else:
            log_error("Missing context.md - run @prd-context first")
    
    return 0 if has_context else 1
