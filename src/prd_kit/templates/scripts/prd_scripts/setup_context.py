#!/usr/bin/env python3
"""PRD Kit - Context Phase Setup Script.

Sets up environment for project context discovery.
"""
...
def main(args: list[str] | None = None) -> int:
    """Main entry point."""
    parser = argparse.ArgumentParser(description="PRD Kit Context Setup")
    ...
    command_file = paths.commands_dir / "context.md"
    ...
    if parsed.json:
        output = {
            ...
            "STATUS": "ready" if has_deliverable else "missing_deliverable",
        }
        ...
        log_info(f"Context: {'exists' if has_context else 'to be created'}")
        log_info(f"Tech Constitution: {'✓' if has_tech_constitution else '✗'}")
        
        if has_deliverable:
            log_success("Context phase ready")
        else:
            log_error("Missing deliverable.md - run @prd-init-feature first")
    
    return 0 if has_deliverable else 1
